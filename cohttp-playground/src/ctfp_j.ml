...
let write_repo_info : _ -> repo_info -> _ = (
  fun ob x ->
...
let string_of_repo_info ?(len = 1024) x =
  let ob = Bi_outbuf.create len in
  write_repo_info ob x;
...
let repo_info_of_string s =
  read_repo_info (Yojson.Safe.init_lexer ()) (Lexing.from_string s)
